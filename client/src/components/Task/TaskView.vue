      ...
      >
        ...
        <v-row>
          ...
          >
            ...
            <v-col cols="4">
            <Select :label="'-- Select user--'" :mode="'user'" :items="group.users" v-on:setUser="setUser" required/>
            </v-col>
        </v-row>
...
export default {
  ...
  methods: {
    ...
    async sendTask() {
      ...
      if (this.task.title.length >= 5 && this.task.description.length >= 15 && this.task.asigned_to != "") {
        try {
          var res = await axios.post(
            this.apiURL + "/task/send-task",
            {
              'task' : this.task,
